...
test("keeps local game data and adult controls in the product source", async () => {
  const [gameApp, gameData, storage, packageJson, styles, sunnyTheme, forestTheme, spaceTheme, levelOneAssets, levelTwoAssets, levelThreeAssets, largeLevelOneAssets, largeLevelTwoAssets, largeLevelThreeAssets] = await Promise.all([
    ...
    readdir(new URL("public/products-large/level-1/", projectRoot)),
    readdir(new URL("public/products-large/level-2/", projectRoot)),
    readdir(new URL("public/products-large/level-3/", projectRoot)),
  ]);
  ...
  assert.match(gameApp, /getFoodImage\(food, true\)/);
  ...
  [largeLevelOneAssets, largeLevelTwoAssets, largeLevelThreeAssets].forEach((assets, index) => {
    const level = index + 1;
    const ids = [...gameData.matchAll(new RegExp(`id: "(l${level}-[^"]+)"`, "g"))]
      .map((match) => `${match[1]}.webp`)
      .sort();
